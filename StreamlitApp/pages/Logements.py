...
def define_map(ville, weather_api_key) :
    ...
    for logement in data:
        ...
        voie  = logement["adresse_nom_voie"]

        if logement["code_postal"] is not None:  
            code = logement["code_postal"]
        else : 
            code = 0
        ...
        folium.Marker(
            location=[lat, lon],
            popup=f"Adresse : {voie}, {int(code)}, {nom}<br>Surface : {surface} m² <br> Prix : {prix}€",
